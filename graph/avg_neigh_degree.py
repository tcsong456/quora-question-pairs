...
def neighbor_deg_graph(hash_data, deg_dict):
    ...
    for _, h1, h2 in hash_data:
        ...
        nei_ratio = nei_max / np.maximum(nei_min, 1)
        nei_mean = (nei_min + nei_max) / 2
        nei_stats.append([nei_min, nei_max, nei_mean, nei_diff, nei_ratio])
...
if __name__ == '__main__':
    ...
    G = build_graph([train_hash, test_hash])
    avg_degrees = nx.average_neighbor_degree(G)
    train_nei_stats = neighbor_deg_graph(train_hash, avg_degrees)
